...
export function renderGeospatialHtml(options = {}) {
  ...
  const heroHtml = investigationHero({
    ...
    title: "Territorial Pressure & Infrastructure Stress",
    subtitle: "Real-coordinate mapping of water dry-outs, grid failures, industrial emissions, and civil liberties actions across all 24 Tunisian governorates.",
    metadata: [
      { label: "TERRITORIAL SCOPE", value: "24 Governorates" },
      { label: "PRIMARY HOTSPOT", value: "Gulf of Gabès / Gafsa Basin" },
      { label: "CARTOGRAPHY", value: "GeoJSON Coordinates (EPSG:4326)" },
      { label: "EPISTEMIC STATUS", value: "Fact-Checked & Geocoded" }
    ],
    summary: "The 404TN Geospatial Monitor cross-references verified field reports, satellite remote-sensing data, utility alerts from SONEDE and STEG, and legal monitoring into an open geographic intelligence record."
  });
  ...
  const govRowsHtml = governorates.map((g, idx) => `
    <tr class="border-b border-surface-800/60 hover:bg-surface-800/30 transition-colors font-mono text-xs">
      <td class="py-2.5 px-3 text-surface-500">${String(idx + 1).padStart(2, '0')}</td>
      <td class="py-2.5 px-3 font-sans font-bold text-bone-100">${escapeHtml(g.name)}</td>
      <td class="py-2.5 px-3 text-surface-400">${escapeHtml(g.code)}</td>
      <td class="py-2.5 px-3 text-crimson font-medium text-right">${g.water}</td>
      <td class="py-2.5 px-3 text-sand font-medium text-right">${g.power}</td>
      <td class="py-2.5 px-3 text-right"><span class="px-1.5 py-0.5 text-[10px] ${g.pollution === 'CRITICAL' ? 'bg-crimson/20 text-crimson border border-crimson/30' : g.pollution === 'HIGH' ? 'bg-amber-900/30 text-amber-300 border border-amber-800/40' : 'bg-surface-800 text-surface-400'}">${escapeHtml(g.pollution)}</span></td>
      <td class="py-2.5 px-3 text-right text-surface-300">${g.protests}</td>
      <td class="py-2.5 px-3 font-sans text-surface-400 text-xs">${escapeHtml(g.status)}</td>
    </tr>
  `).join('');

  return `
    <div id="geospatial-view" class="space-y-12 pb-24">
      
      <!-- HERO -->
      ${heroHtml}

      <!-- MAIN CARTOGRAPHIC WORKSPACE -->
      <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-surface-800">
          <div>
            ...
            ${sectionHeading("Live Geographic Projection & Incident Layering")}
          </div>
          ...
          <div class="lg:col-span-8 bg-surface-900/60 border border-surface-800 p-4 sm:p-6 space-y-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 pt-2 text-[11px] font-mono text-surface-400 border-t border-surface-800/60">
              <div class="flex items-center gap-4">
                <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-crimson"></span> Critical Stress</span>
                <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-amber-400"></span> Medium Stress</span>
                <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-emerald-400"></span> Baseline Monitor</span>
              </div>
          ...
          <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-surface-900/60 border border-surface-800 p-6 space-y-4">
              ...
              <div class="pt-3 border-t border-surface-800 flex items-center justify-between text-xs font-mono">
                <span class="text-surface-400">Coordinates: 33.88°N, 10.09°E</span>
            ...
            <div class="bg-surface-900/60 border border-surface-800 p-6 space-y-4">
              ...
              <div class="pt-3 border-t border-surface-800 flex items-center justify-between text-xs font-mono">
                <span class="text-surface-400">Coordinates: 34.42°N, 8.78°E</span>
      ...
      <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="pb-4 border-b border-surface-800">
          ...
          ${sectionHeading("24 Governorates: Documented Pressure Metrics")}
          <p class="text-xs sm:text-sm text-surface-400 font-light max-w-3xl mt-2">
        ...
        <div class="overflow-x-auto border border-surface-800 bg-surface-900/40">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-surface-800 bg-surface-900/80 font-mono text-[11px] uppercase tracking-wider text-surface-400">
                ...
                <th class="py-3 px-3 text-right text-sand">Grid Alerts</th>
      ...
      <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="p-6 bg-surface-900/60 border border-surface-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="space-y-1">
            <span class="text-[10px] font-mono uppercase tracking-widest text-sand font-bold">GEOSPATIAL METHODOLOGY</span>
            <p class="text-xs text-surface-400 font-light">
          ...
          <div class="flex items-center gap-4 text-xs font-mono shrink-0">
            <a href="/evidence" class="text-surface-300 hover:text-crimson transition-colors">Evidence Register →</a>
            <a href="/methodology" class="text-surface-300 hover:text-crimson transition-colors">Verification Protocol →</a>
          </div>
        </div>
      </section>
