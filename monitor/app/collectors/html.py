# monitor/app/collectors/html.py
import time
import re
...
class HTMLCollector(BaseCollector):
    """Reusable generic HTML listing-page collector with link discovery and article parsing."""
        ...
        try:
            with self.get_http_client() as client:
                ...
                for listing_url in self.urls:
                    try:
                        res = self.fetch_with_retry(client, listing_url)
                        ...
                        if res.status_code == 200:
                            ...
                            for a in soup.find_all("a", href=True):
                                ...
                                # Internal domain links that look like articles or news items
                                if parsed_link.netloc == domain or not parsed_link.netloc:
                                    path = parsed_link.path.lower()
                                    if any(p in path for p in ["/article", "/actualite", "/news", "/publication", "/communique", "/rapport", "/dossier", "/post"]):
                                        discovered_links.add(full_url)
                                    elif re.search(r"/\d{4}/\d{2}/", path) or re.search(r"-\d+\.html?", path) or re.search(r"/\d{4,}", path):
                                        discovered_links.add(full_url)

                    except Exception as exc:
                        metrics.last_error = f"Discovery error on {listing_url}: {str(exc)}"
                ...
                for art_url in selected_links:
                    try:
                        ...
                        cand = self._parse_html_article(art_res.text, art_url)
    ...
    def _parse_html_article(self, html_text: str, source_url: str) -> Optional[NormalizedCandidate]:
